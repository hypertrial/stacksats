...
The strategy you write here targets the Stacking Sats problem: dynamically allocating a fixed Bitcoin accumulation budget across a fixed horizon, then testing whether the result robustly acquires more BTC than uniform DCA. For the full framing, see [The Stacking Sats Problem](../index.md#the-stacking-sats-problem).
