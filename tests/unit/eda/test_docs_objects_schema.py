...
from stacksats.strategy_time_series import BRKLineageSpec, WeightTimeSeries


def test_brk_lineage_targets_documented_columns() -> None:
    WeightTimeSeries.validate_brk_lineage_coverage()


def test_brk_passthrough_columns_have_schema_specs() -> None:
    specs = WeightTimeSeries.schema_dict()
    for column in WeightTimeSeries.BRK_BTC_CSV_COLUMNS:
        assert column in specs


def test_brk_btc_columns_have_lineage_rows() -> None:
    lineage_sources = {item.source_column for item in WeightTimeSeries.BRK_LINEAGE}
    for column in WeightTimeSeries.BRK_BTC_CSV_COLUMNS:
...
def test_lineage_coverage_raises_when_source_column_missing(
    ...
    monkeypatch.setattr(
        WeightTimeSeries,
        "BRK_SOURCE_COLUMNS",
        WeightTimeSeries.BRK_SOURCE_COLUMNS + ("MissingSourceColumn",),
    )
