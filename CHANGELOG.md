...
### Changed
- **Pandas → Polars migration (core complete):** (1) Timestamps replaced with `datetime.datetime` in `StrategySeriesMetadata`, `StrategyContext`, `DayState`, `WeightTimeSeriesBatch`, prelude, and framework contract. (2) Data layer: `BTCDataProvider` and `ColumnMapDataProvider` return Polars DataFrames. (3) `FeatureTimeSeries.from_dataframe(pl.DataFrame)` added; `StrategyContext.from_features_df` accepts Polars or pandas. (4) `WeightTimeSeries` and `WeightTimeSeriesBatch` use Polars internally; `to_dataframe()` returns `pl.DataFrame`; `from_dataframe`/`from_flat_dataframe` accept Polars or pandas. (5) Strategy time-series analysis and diagnostics mixins rewritten for Polars. (6) `model_development_helpers` (zscore, rolling_percentile, compute_mvrv_volatility) preserve pandas index when converting Polars results back. Strategy hooks (`transform_features`, `build_signals`, `build_target_profile`) continue to use pandas. Remaining: remove `pandas` from dependencies; update docs migration note.
