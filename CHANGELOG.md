...
- Documentation now leads with `stacksats strategy compare` for multi-strategy comparison (strategies reference, model-development helpers, maintainer workflow, backtest/export next steps, README map) and keeps `scripts/compare_strategies.py` as a checkout-oriented alias.
...
- Reworked the package roots into stable façade packages: `stacksats.api`, `stacksats.backtest`, `stacksats.cli`, `stacksats.eda`, `stacksats.export_weights`, `stacksats.model_development`, `stacksats.runner`, `stacksats.statistical_validation`, `stacksats.strategy_time_series`, and `stacksats.strategy_types` now delegate to smaller internal modules while preserving their documented import paths and CLI entrypoints.
- Reorganized `tests/unit/` to mirror runtime domains (`api`, `architecture`, `eda`, `execution`, `features`, `package`, `runner`, `scripts`, `service`, `strategy_time_series`, `strategy_types`, `viz`, and others) and added AST-based import-boundary tests for façade modules and cross-package layering rules.
