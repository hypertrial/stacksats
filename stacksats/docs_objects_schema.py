"""Helpers for keeping WeightTimeSeries schema docs in sync with code."""
...
def objects_docs_path(root_dir: Path | None = None) -> Path:
    """Return generated WeightTimeSeries schema docs path for repository root."""
...
def render_objects_docs(content: str) -> str:
    """Return schema docs content with generated WeightTimeSeries sections."""
    updated = _replace_section(
        content,
        TIMESERIES_SCHEMA_BEGIN,
        TIMESERIES_SCHEMA_END,
        WeightTimeSeries.schema_markdown_table(),
    )
    return _replace_section(
        updated,
        TIMESERIES_LINEAGE_BEGIN,
        TIMESERIES_LINEAGE_END,
        WeightTimeSeries.brk_lineage_markdown(),
    )
